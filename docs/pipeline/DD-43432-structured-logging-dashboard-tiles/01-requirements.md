...
| **FR-006** | **Log lines that drive a tile become a consumed interface.** Every log line that a dashboard KQL query matches on — the four existing Tile-1 lines (FR-002 – FR-004), the new `WAITING_FOR_UPLOAD` line (FR-001), and whatever Tile-2 lines FR-005 settles on — must be identifiable by a **stable** predicate. From this ticket onwards, changing the text, level, field names or emission point of any of these lines requires the corresponding file under `support/dashboard-kql` to be updated in the same change. This is a documented convention, not a build-enforced one (OQ-007 covers whether any automated check is expected). **Revisited 2026-09-24, raised at Code Review on PR #231:** of the 7 log-line markers across both `.kql` files, only `WAITING_FOR_UPLOAD` has any test, and even that test (`IdpcAvailabilityServiceTest`) duplicates the marker string by hand rather than reading it from `support/dashboard-kql/ingestion-phase-counts.kql` — so a rewording of any of the 7 lines, including that one, would leave CI green while a tile silently shows 0. OQ-007's "no automated enforcement is expected" is reopened by this — see OQ-013. |
...
- **OQ-013 (no contract test for the log-line ↔ KQL-marker coupling — added 2026-09-24, raised at Code Review on PR #231; ticket raised 2026-09-24 as [DD-43672](https://hmcts.atlassian.net/browse/DD-43672)):** FR-006 requires the 7 log-line markers driving both tiles to stay in sync with `support/dashboard-kql/*.kql`, but nothing enforces this. Only `WAITING_FOR_UPLOAD` (1 of 7) has any test at all, and that test (`IdpcAvailabilityServiceTest`) hardcodes its own copy of the marker string rather than reading it from the `.kql` file — so even that one test cannot catch the `.kql` file and the Java log line drifting apart from each other, only the Java line drifting from the test's independent copy. A reviewer proposed a small contract test: read each marker from `support/dashboard-kql/*.kql` and assert it is still actually logged somewhere. This reopens OQ-007's Stage-2 resolution ("no automated enforcement is expected") rather than replacing it outright. Tracked as [DD-43672](https://hmcts.atlassian.net/browse/DD-43672), not fixed inline on DD-43470/DD-43471 — its own input brief is `docs/pipeline/DD-43672-dashboard-kql-contract-test/00-input-brief.md`. It should close **before Story 3** wires the tiles up, so a silent drift isn't shipped to a live dashboard before anything catches it. — Owner: requester · Due: before Story 3 (DD-43472).
...
open items (OQ-001, OQ-008, OQ-009, OQ-010, OQ-011, OQ-012, OQ-013) are non-blocking
confirmations/exclusions, not open design questions. Proceeding to Stage 2 (Architecture & Design) on
