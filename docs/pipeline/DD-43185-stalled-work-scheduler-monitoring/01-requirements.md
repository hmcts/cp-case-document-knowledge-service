...
- **OQ-009 (EXPLAIN-plan evidence — deliverable or manual step?):** the AC says "evidenced by an EXPLAIN plan attached to the ticket". **Row-count half resolved — 2026-09-01:** the requester has confirmed both `case_documents` and `case_query_status` hold fewer than ~100,000 rows in production — see ADR-003's Consequences for the full resolution and its effect on the `V1014` migration's DBA sign-off gate. At that confirmed volume, `StalledWorkQueryPlanTest`'s ~100k-row Testcontainers plan assertion is a representative proxy for production, not merely a synthetic stand-in, so the 500 ms target is now credibly (if not literally, production-environment) evidenced in-repo. Still open: a formal one-off production `EXPLAIN (ANALYZE, BUFFERS)` capture attached to the Jira ticket remains good practice for the permanent record, but is no longer a merge blocker. — Owner: requester + platform/DBA · Due: post-deploy, non-blocking.
