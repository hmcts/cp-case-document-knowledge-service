...
-- ----------------------------------------------------------------------------
-- 1) Version generators
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION next_case_level_latest_doc_answer_version(p_case_id UUID, p_query_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION next_case_level_all_documents_answer_version(p_case_id UUID, p_query_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION next_defendant_answer_version(p_case_id UUID, p_query_id UUID, p_defendant_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  SELECT ('x' || substr(md5(p_case_id::text), 1, 8))::bit(32)::int,
         ('x' || substr(md5(p_query_id::text), 1, 8))::bit(32)::int,
         ('x' || substr(md5(p_defendant_id::text), 1, 8))::bit(32)::int
    INTO k1, k2, k3;

  PERFORM pg_advisory_xact_lock(k1, k2 + k3);
  ...
  SELECT COALESCE(MAX(a.version),0) + 1
   ...
   WHERE a.case_id = p_case_id
     AND a.query_id = p_query_id
     AND a.defendant_id = p_defendant_id;
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS case_level_latest_doc_answer_reservations (
    reservation_id UUID PRIMARY KEY DEFAULT uuid_v4(),
    case_id UUID NOT NULL,
    query_id UUID NOT NULL,
    ...
    CONSTRAINT uq_cllda_res UNIQUE (case_id, query_id, doc_id),
    CONSTRAINT fk_cllda_ar_query FOREIGN KEY (query_id) REFERENCES queries (query_id) ON DELETE CASCADE,
    CONSTRAINT fk_cllda_ar_doc   FOREIGN KEY (doc_id)   REFERENCES case_documents (doc_id) ON DELETE CASCADE
);
CREATE INDEX idx_cllda_ar_status ON case_level_latest_doc_answer_reservations (status);
CREATE INDEX idx_cllda_ar_case_query ON case_level_latest_doc_answer_reservations (case_id, query_id);

CREATE TABLE IF NOT EXISTS case_level_all_documents_answer_reservations (
    reservation_id UUID PRIMARY KEY DEFAULT uuid_v4(),
    case_id UUID NOT NULL,
    query_id UUID NOT NULL,
    version INTEGER NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_clada_res UNIQUE (case_id, query_id),
    CONSTRAINT fk_clada_ar_query FOREIGN KEY (query_id) REFERENCES queries (query_id) ON DELETE CASCADE
);
CREATE INDEX idx_clada_ar_status ON case_level_all_documents_answer_reservations (status);
CREATE INDEX idx_clada_ar_case_query ON case_level_all_documents_answer_reservations (case_id, query_id);

CREATE TABLE IF NOT EXISTS defendant_answer_reservations (
    reservation_id UUID PRIMARY KEY DEFAULT uuid_v4(),
    case_id UUID NOT NULL,
    query_id UUID NOT NULL,
    doc_id UUID NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_def_res UNIQUE (case_id, query_id, doc_id, defendant_id),
    CONSTRAINT fk_def_ar_query FOREIGN KEY (query_id) REFERENCES queries (query_id) ON DELETE CASCADE,
    CONSTRAINT fk_def_ar_doc   FOREIGN KEY (doc_id)   REFERENCES case_documents (doc_id) ON DELETE CASCADE
);
CREATE INDEX idx_def_ar_status ON defendant_answer_reservations (status);
CREATE INDEX idx_def_ar_case_query ON defendant_answer_reservations (case_id, query_id);
